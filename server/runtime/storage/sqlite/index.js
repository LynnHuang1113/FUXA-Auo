...
function DaqNode(_settings, _log, _id) {
    ...
    function _getTagValues(db, id, fromts, tots) {
        return new Promise(function (resolve, reject) {
            ...
            var sql = `SELECT MAX(datetime(dt / 1000, 'unixepoch', 'localtime')) AS dt, id, value
            FROM data WHERE id = ? AND dt BETWEEN ? AND ?GROUP BY strftime('%Y-%m-%d %H:%M:%S', dt / 1000, 'unixepoch', 
            'localtime'), id, value ORDER BY dt DESC`;
